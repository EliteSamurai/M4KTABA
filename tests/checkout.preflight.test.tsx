...
describe('checkout preflight drift', () => {
  ...
  test('drift → banner → accept → intent created', async () => {
    ...
    (global as any).fetch = jest.fn(
      async (url: RequestInfo | URL, init?: any) => {
        ...
        if (href.includes('/api/cart/review')) {
          return {
            ...
            json: async () => ({
              hasDrift: true,
              reviewed: [
                { id: '1', title: 'Test Book', price: 11, quantity: 2 },
              ],
              changes: [
                {
                  id: '1',
    ...
    const btn = await screen.findByRole('button', {
      name: /validate|continue|address/i,
    });
    ...
    await waitFor(() => {
      expect(seq.some(u => u.includes('/api/create-payment-intent'))).toBe(
        true
      );
    });
  });
