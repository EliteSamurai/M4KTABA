...
jest.mock("react-hook-form", () => {
  ...
  const notify = () => Array.from(store.listeners).forEach((l) => {
    try { l(); } catch {}
  });
  return {
    useForm: () => ({
      ...
      reset: (vals) => { if (vals && typeof vals === "object") { store.values = { ...store.values, ...vals }; notify(); } },
      setValue: (name, value) => { store.values[name] = value; notify(); },
      getValues: () => ({ ...store.values }),
      formState: { isValid: true },
    }),
    useWatch: ({ name }) => store.values[name] ?? "",
    useFormContext: () => ({ getFieldState: () => ({}), formState: { isValid: true } }),
    Controller: ({ name, render }) => {
      ...
      return render({
        field: {
          ...
          onChange: (e) => { const next = e && e.target ? e.target.value : e; store.values[name] = next; notify(); },
          onBlur: () => {},
          ref: () => {},
        },
      });
    },
    FormProvider: ({ children }) => React.createElement(React.Fragment, null, children),
  };
});
