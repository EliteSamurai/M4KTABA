...
(async () => {
  ...
  try {
    ...
    await page.goto(`${baseUrl}/checkout?synth=1`, { waitUntil: 'networkidle' });
    ...
    const synthDetected = await page.evaluate(() => {
      return window.location.search.includes('synth=1') || 
    ...
    try {
      ...
      try {
        ...
        console.log('⚠️  Text content not found, trying alternative selectors...');
        try {
