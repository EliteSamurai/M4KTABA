...
(async () => {
  ...
  try {
    ...
    console.log(`🔍 Navigating to: ${baseUrl}/checkout`);
    ...
    if (!redirectedUrl.includes('callbackUrl=%2Fcheckout')) {
      throw new Error(`Expected callbackUrl=/checkout in redirect, got: ${redirectedUrl}`);
    }
    ...
    if (email && password) {
      ...
      console.log('ℹ️ Skipped authenticated checkout render test. Set SYNTH_CHECKOUT_EMAIL and SYNTH_CHECKOUT_PASSWORD.');
    }
  } catch (error) {
